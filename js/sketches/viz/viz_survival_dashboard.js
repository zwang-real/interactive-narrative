...
(function () {
    ...
    function buildLayout(p, manager) {
        ...
        var chartW = W - chartX + (manager.offsetX || 0) - (isCuisine() ? 20 : 88);
    ...
    function buildBars(chartX, chartY, chartW, chartH) {
        ...
        if (isCuisine()) {
            var labelW  = 110;
            var pctW    = 44;
            var barMaxW = chartW - labelW - pctW;
            var barH    = Math.max(14, Math.min(22, (chartH - 16) / n - 3));
            var gap     = Math.min(7, Math.max(2, (chartH - 16 - n * barH) / Math.max(n - 1, 1)));
            var totalH  = n * barH + (n - 1) * gap;
            var startY  = chartY + Math.max(0, (chartH - totalH) / 2);
            ...
            bars = rows.map(function (d, i) {
                ...
                return {
                    ...
                    labelX: chartX,
                    labelW: labelW,
                    chartBottom: startY + totalH,
                    rate: d.survival_rate, label: d.label, count: d.count, key: key,
                    horizontal: true,
                };
            });
        } else {
            var gap2   = n <= 2 ? 64 : (n > 10 ? 4 : (n > 6 ? 8 : 18));
            var barW   = n ? Math.min(40, (chartW - gap2 * (n + 1)) / n) : 40;
            var totalW = n * barW + (n - 1) * gap2;
            var startX = chartX + (chartW - totalW) / 2;
            var maxH   = chartH - 34 - 24;

            bars = rows.map(function (d, i) {
                var key = activeVar + '_' + i;
                if (animProgress[key] === undefined) animProgress[key] = 0;
                return {
                    x: startX + i * (barW + gap2), y: chartY,
                    w: barW, maxH: maxH,
    ...
    function drawBars(p, dt, manager) {
        ...
        if (bars[0].horizontal) {
            drawHorizontalBars(p, dt, manager);
        } else {
            drawVerticalBars(p, dt, manager);
        }
    }

    function drawHorizontalBars(p, dt, manager) {
        var refX = bars[0].x + bars[0].maxW * 0.5;

        p.stroke(COL_BORDER); p.strokeWeight(1);
        p.drawingContext.setLineDash([4, 4]);
        p.line(refX, bars[0].y - 4, refX, bars[bars.length-1].y + bars[bars.length-1].h + 4);
        p.drawingContext.setLineDash([]);
        p.noStroke();
        p.fill(COL_MUTED); p.textFont('IBM Plex Mono'); p.textSize(11);
        p.textAlign(p.CENTER, p.BOTTOM);
        p.text('50%', refX, bars[0].y - 6);

        bars.forEach(function (bar, i) {
            animProgress[bar.key] = lerp(animProgress[bar.key], 1, dt * 4);
            var bw = bar.maxW * bar.rate * animProgress[bar.key];

            var col = p.color(bar.rate >= 0.5 ? COL_HIGH : COL_LOW);
            col.setAlpha(i === hoveredBar ? 255 : 200);
            p.noStroke(); p.fill(col);
            p.rect(bar.x, bar.y, bw, bar.h, 0, 3, 3, 0);

            p.fill(bar.rate >= 0.5 ? COL_TEXT : COL_MUTED);
            p.textFont('IBM Plex Mono'); p.textSize(12);
            p.textAlign(p.RIGHT, p.CENTER); p.noStroke();
            p.text(bar.label, bar.x - 6, bar.y + bar.h / 2);

            p.fill(COL_TEXT); p.textSize(11);
            p.textAlign(p.LEFT, p.CENTER);
            p.text(pct(bar.rate), bar.x + bar.maxW * bar.rate + 5, bar.y + bar.h / 2);
        });

        // tooltip drawn after all bars so it sits on top
        if (hoveredBar >= 0 && hoveredBar < bars.length) {
            drawTooltip(p, bars[hoveredBar], manager);
        }
    }

    function drawVerticalBars(p, dt, manager) {
        ...
        bars.forEach(function (bar, i) {
            ...
            p.fill(COL_TEXT); p.textFont('IBM Plex Mono'); p.textSize(FONT_LABEL);
            p.textAlign(p.CENTER, p.BOTTOM); p.noStroke();
            p.text(pct(bar.rate), bar.x + bar.w / 2, barTop - 2);

            p.fill(COL_MUTED); p.textSize(FONT_SMALL);
    ...
    function drawTooltip(p, bar, manager) {
        ...
        if (bar.horizontal) {
            var bw = bar.maxW * bar.rate;
            tx = bar.x + bw / 2;
            ty = bar.y - th - 4;
            if (ty < (manager.offsetY || 0) + th + 4) ty = bar.y + bar.h + th + 4;
        } else {
            tx = bar.x + bar.w / 2;
            ty = bar.chartBottom - bar.maxH * bar.rate - 28;
            if (ty - th < 4) ty = th + 4;
        }
        ...
        p.fill(255); p.textFont('IBM Plex Mono'); p.textSize(FONT_SMALL);
        p.textAlign(p.CENTER, p.CENTER);
        p.text(bar.label + ': ' + pct(bar.rate), tx, ty - th * 0.65);
        p.fill(180); p.textSize(FONT_SMALL);
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.y + b.h) {
                if (activeVar !== b.key) {
                    activeVar = b.key;
                    var rows = (survivalData || {})[activeVar] || [];
                    rows.forEach(function (_, j) {
                        var key = activeVar + '_' + j;
                        if (animProgress[key] === undefined) animProgress[key] = 0;
                    });
    ...
    function checkBarHover(mx, my) {
        ...
        for (var i = 0; i < bars.length; i++) {
            var b = bars[i];
            if (b.horizontal) {
                if (mx >= b.labelX && mx <= b.x + b.maxW && my >= b.y && my <= b.y + b.h) {
                    hoveredBar = i; break;
                }
            } else {
                if (mx >= b.x && mx <= b.x + b.w && my >= b.y && my <= b.chartBottom) {
                    hoveredBar = i; break;
                }
            }
        }
