...
(function () {
    var REVIEW_BINS = [0, 100, 250, 500, 750, 1000, 1500, 2500, 5000, 100000];
    ...
    function formatReviewBin(index) {
        ...
        if (index === REVIEW_BINS.length - 2) return formatCompactNumber(REVIEW_BINS[index]) + '+';
    ...
    window.VizYelpHeatmap = {
        prepareData: function (rows) {
            ...
            rows.forEach(function (row) {
                ...
                if (colIndex === -1 && reviewCount >= REVIEW_BINS[REVIEW_BINS.length - 2]) colIndex = REVIEW_BINS.length - 2;
        ...
        draw: function (p, manager) {
            ...
            var rightGridLeft = left + panelW + gutter + 78;
            ...
            function drawPanel(panelLeft, title, key, baseHex) {
                ...
                p.text(title, panelLeft + gridW / 2, top - 6);
                ...
                for (var c = 0; c < cols; c++) {
                    ...
                    p.textAlign(p.CENTER, p.TOP);
                    p.text(formatReviewBin(c), x + cellW / 2, gridTop + gridH + 10);
                }
            ...
            p.noStroke();
            p.fill(255);
            p.rect(left - 22, top - 24, w + 44, h + 108);
            ...
            p.textSize(16);
            p.textAlign(p.CENTER, p.TOP);
            p.text('review count bins', left + w / 2, gridTop + gridH + 80);

            p.push();
            p.translate(left - 44, gridTop + gridH / 2);
            p.rotate(-p.HALF_PI);
            p.text('rating bins', 0, 0);
            p.pop();
            ...
            p.textSize(11);
            p.fill(45);
            p.text('Fewer restaurants to more restaurants', legendX, legendY - 4);
            p.noStroke();
            for (var i = 0; i < steps; i++) {
                var lt = i / (steps - 1);
                p.fill(countColor(p, lt, OPEN_HEX));
                p.rect(legendX + lt * legendW, legendY, legendW / steps + 1, 10);
            }
            p.fill('#4f4a45');
            p.textAlign(p.LEFT, p.TOP);
            p.text('low', legendX, legendY + 12);
            p.textAlign(p.RIGHT, p.TOP);
            p.text(data.maxCount ? String(data.maxCount) + ' in a bin' : 'high', legendX + legendW, legendY + 12);

            p.textAlign(p.RIGHT, p.BOTTOM);
            p.textSize(12);
            p.fill(70);
            p.text('Open: ' + data.totalOpen + '  |  Closed: ' + data.totalClosed, left + w, top + h + 34);
            p.pop();
        }
    };
})();
